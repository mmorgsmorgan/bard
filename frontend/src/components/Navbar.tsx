...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="ui-pressable flex items-center gap-1.5 border px-2.5 py-1.5 font-mono text-[11px] tracking-wide"
...
export function Navbar() {
  ...
  return (
    <>
      <nav
        className="bard-chrome fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 py-3"
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between relative">
          {/* Logo */}
          <Link href="/" className="ui-pressable flex items-center gap-2.5 group shrink-0">
          ...
          <div className="hidden md:flex items-center justify-center flex-1 min-w-0 gap-6 px-4">
            {showLinks && NAV_LINKS.map((l) => (
              <Link
                ...
                className="bard-nav-link font-mono text-[13px]"
              >
                {l.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div
              className="bard-status-chip hidden lg:flex items-center gap-2 px-3 py-1.5 border"
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="ui-pressable flex items-center gap-2 border px-3 sm:px-4 py-2"
                    style={{ borderColor: 'var(--rule)' }}
                    aria-haspopup="menu"
                  ...
                  >
                    ...
                    <svg
                      className={`motion-chevron w-3 h-3 ${showDropdown ? 'rotate-180' : ''}`}
                  ...
                  <div
                    ...
                    className="bard-popover absolute right-0 mt-1 w-48 p-1 z-50"
                  >
                      {[
                        ...
                        <Link
                          ...
                          className="bard-menu-item ui-pressable flex items-center gap-3 w-full px-3 py-2.5 font-mono text-[13px]"
            ...
            {showLinks && (
              <button
                className="ui-pressable md:hidden flex items-center justify-center w-9 h-9 border"
                style={{ borderColor: 'var(--rule)', color: 'var(--ink)' }}
                onClick={() => setMobileOpen((v) => !v)}
                aria-label="Menu"
                aria-expanded={mobileOpen}
                aria-controls="bard-mobile-menu"
              >
        ...
        {showLinks && (
          <div
            ...
            className="bard-mobile-menu md:hidden"
          >
            <div className="bard-mobile-menu-inner mt-3 pt-3 flex flex-col gap-1">
              {NAV_LINKS.map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
                  ...
                  aria-current={pathname === l.href || pathname.startsWith(`${l.href}/`) ? 'page' : undefined}
                  data-active={pathname === l.href || pathname.startsWith(`${l.href}/`)}
                  className="bard-mobile-link ui-pressable font-mono text-sm py-2.5 px-1"
                >
                  {l.label}
                </Link>
              ))}
      ...
      {mcpSetupOpen && (
        <div
          className="bard-modal-layer fixed inset-0 z-[70] flex items-center justify-center overscroll-none p-3 sm:p-6"
          role="presentation"
          onMouseDown={() => setMcpSetupOpen(false)}
        >
          <div className="bard-modal-scrim absolute inset-0 bg-black/60" aria-hidden />
          <div
            ref={mcpDialogRef}
            role="dialog"
            aria-modal="true"
            ...
            tabIndex={-1}
            data-lenis-prevent
            className="bard-modal-surface relative z-10 flex max-h-[calc(100dvh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden overscroll-contain border outline-none sm:max-h-[calc(100dvh-3rem)]"
          ...
          >
            <div className="flex items-start justify-between gap-6 border-b px-5 py-4 sm:px-6" style={{ borderColor: 'var(--rule)' }}>
              ...
              <button
                ...
                className="ui-pressable flex h-8 w-8 shrink-0 items-center justify-center border font-mono text-lg hover:border-[var(--accent)]"
                style={{ borderColor: 'var(--rule)', color: 'var(--ink)' }}
